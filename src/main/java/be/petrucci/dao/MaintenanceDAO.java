...
public class MaintenanceDAO extends DAO<Maintenance> {
	...
	public boolean delete(Maintenance obj) {
		try {
			...
			ClientResponse res = this.getResource().path("maintenance").accept(MediaType.APPLICATION_JSON)
	...
	public boolean update(Maintenance obj) {
		try {
			...
			ClientResponse res = this.getResource().path("maintenance").type(MediaType.APPLICATION_JSON)
	...
	public ArrayList<Maintenance> findAll() {
		...
		try {
			String response = getResource().path("maintenance").accept(MediaType.APPLICATION_JSON).get(String.class);
