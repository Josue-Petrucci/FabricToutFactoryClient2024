...
public class UserDAO extends DAO<User> {
	...
	public User find(User obj) {
		...
		try {
			ClientResponse res = this.getResource().path("user").type(MediaType.APPLICATION_JSON)
			...
			if (res.getStatus() == 200) {
				...
				User user = new User(id, lastname, firstname, age, address, obj.getMatricule(), obj.getPassword());
				return user;
			}
		} catch (Exception ex) {
			ex.printStackTrace();
