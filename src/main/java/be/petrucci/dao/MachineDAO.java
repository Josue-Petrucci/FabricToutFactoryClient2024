...
public class MachineDAO extends DAO<Machine> {
	...
	public boolean create(Machine obj) {
		try {
			...
			ClientResponse res = this.getResource().path("machine").type(MediaType.APPLICATION_JSON)
	...
	public boolean delete(Machine obj) {
		try {
			ObjectMapper objectMapper = new ObjectMapper();
			String json = objectMapper.writeValueAsString(obj);
			ClientResponse res = this.getResource().path("machine").accept(MediaType.APPLICATION_JSON)
	...
	public ArrayList<Machine> findAll() {
		String responseJSON = this.getResource().path("machine").accept(MediaType.APPLICATION_JSON).get(String.class);
