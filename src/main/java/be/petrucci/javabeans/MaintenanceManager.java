...
public class MaintenanceManager extends User implements Serializable {
	...
	private boolean paramsAreValid() {
		...
		if (this.getMatricule() == null
				|| (!this.getMatricule().startsWith("Admin-") && !this.getMatricule().startsWith("MMana-")
						&& !this.getMatricule().startsWith("MWork-") && !this.getMatricule().startsWith("PuEmp-"))) {
			return false;
		}
		if (this.getPassword() == null || this.getPassword().length() < 5 || !this.getPassword().matches("^[A-Z]+$")) {
			return false;
		}
