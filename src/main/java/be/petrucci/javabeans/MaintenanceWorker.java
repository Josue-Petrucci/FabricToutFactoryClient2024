...
public class MaintenanceWorker extends User implements Serializable {
	...
	public static ArrayList<MaintenanceWorker> giveSelectedWorkers(ArrayList<MaintenanceWorker> workerList,
			String[] ids) {
		ArrayList<MaintenanceWorker> selectedWorker = new ArrayList<>();
		for (String workerId : ids) {
			int id = Integer.parseInt(workerId);
			MaintenanceWorker matchingWorker = workerList.stream().filter(w -> w.getId() == id).findFirst()
					.orElse(null);
	...
	public void getInProgressMaintenances(ArrayList<Maintenance> maintenanceList) {
		setMaintenances(new ArrayList<Maintenance>(maintenanceList.stream().filter(m -> {
